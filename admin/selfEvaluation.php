                ...
                <div class="row">
                    <div class="col-12 mb-30">
                        <div class="card-box pd-30 pt-10 height-100-p">
                            <h2 class="mb-30 h4">PART A: Target Evaluation</h2>
                            <div class="pb-20">
                                        ...
                                        <table id="objectivesTable" class="data-table table stripe">
                                            ...
                                            <tbody>
                                                ...
                                                $user_id = $_SESSION['userId'];
                                                    ...
                                                    $sql = "SELECT * FROM (
                                                                SELECT id, objective_code, details, period, kpi_indicator, unit_of_measure, target 
                                                                FROM employee_objectives where user_id='$user_id'
                                                                and period='$get_periodCode'
                                                                UNION 
                                                                SELECT CONCAT('C', id) AS id, code, description, period, '', '', '' 
                                                    ...
                                                    if ($query->rowCount() > 0) {
                                                        foreach ($results as $result) { ?>  
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <input type="number" name="scores[<?php echo $result->id; ?>]" class="form-control" required>
        ...
        $(document).ready(function() {
            if (!$.fn.DataTable.isDataTable('#objectivesTable')) {
                $('#objectivesTable').DataTable({
                    "order": [[0, "asc"]] // Default sort by the first column (ID) in ascending order
                });
            }
        });
    </script>
