...
if (isset($_POST['submit_scores'])) {
    ...
    $evaluator_id = $supervisor_id; // Supervisor's user ID

    foreach ($_POST['scores'] as $evaluation_item_code => $evaluator_score) {
        ...
        if (!empty($subordinate_user_id)) {
            $sql = "UPDATE employee_rating er
                    ...
                    SET er.evaluator_id='$evaluator_id', 
                        er.evaluator_evaluation_date='$evaluator_evaluation_date', 
                        er.evaluator_score='$evaluator_score', 
                        er.evaluator_remarks='$remarks' 
    ...
    echo "<script type='text/javascript'> document.location = 'supervisorEvaluationList.php'; </script>";
}
?>
                                                    ...
                                                    if ($query->rowCount() > 0) {
                                                        foreach ($results as $result) { ?>  
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <input type="text" name="remarks[<?php echo $result->id; ?>]" class="form-control">
                                                                </td>
                                                            </tr>
                                                        <?php $cnt++; }
                                                    }
